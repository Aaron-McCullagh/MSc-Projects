...
$subcatendpoint = "http://amccullagh06.lampt.eeecs.qub.ac.uk/api/sportzone.php?subcat=$subcategory";

$dataset = file_get_contents($subcatendpoint);

$allsubcatproducts = json_decode($dataset, true);
...
echo "<div class = 'container-fluid'>
 ...
 <div class='sortBy'>
    ...
    <select class='form-select' aria-label='Default select example'onchange='location = this.value;'>
        ...
        <option value ='subcategorysort.php?category=$categoryfilter&subcat=$subcategory&search=price'>Price (Low to High)</option>
        <option value='subcatreversesort.php?category=$categoryfilter&subcat=$subcategory&search=price'>Price (High to Low)</option>
        <option value='subcategorysort.php?category=$categoryfilter&subcat=$subcategory&search=brand'>Brand (A to Z)</option>
        <option value='subcatreversesort.php?category=$categoryfilter&subcat=$subcategory&search=brand'>Brand (Z to A)</option>
...
<div class='sortBy'>
    ...
    <select class='form-select' aria-label='Default select example'onchange='location = this.value;'>
        ...
        if($categoryfilter=='Basketball'){
            ...
            <option value ='subcategory.php?category=$categoryfilter&subcat=Other-Camping-Equipment'>Other</option>            
            ";
...
<div class='container'>
    <div class='row'>
        ...
        foreach ($allsubcatproducts as $row) {
...
<div class="container">
    <hr>
    <h5><strong>Most viewed.. </strong></h5><br>
    ...
    <div class="carousel">
        ...
        <?php
        $viewedendpoint = "http://amccullagh06.lampt.eeecs.qub.ac.uk/api/sportzone.php?mostviewed=$subcatid";
        $vieweddataset = file_get_contents($viewedendpoint);
        $viewedproducts = json_decode($vieweddataset, true);

        foreach ($viewedproducts as $row) {
